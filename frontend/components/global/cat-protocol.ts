...
function crc8(data: Uint8Array) {
  let crc = 0;
  for (const byte of data) crc = crc8_table[(crc ^ byte) & 0xff];
  return crc & 0xff;
}
...
export class CatPrinter {
  ...
  pend(data: Uint8Array) {
    for (let i = 0; i < data.length; ++i) this.buffer[this.bufferSize++] = data[i];
  }
  ...
  async send(data: Uint8Array) {
    if (this.bufferSize + data.length > this.mtu) await this.flush();
    this.pend(data);
  }
